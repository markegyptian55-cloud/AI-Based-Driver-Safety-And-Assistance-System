...
def process_video(
    ...
    *,
    ...
    def _frames():
        """Source frames after stride, as (frame, t) pairs."""
        for src_idx, ft in enumerate(iter_frames(video_path, max_seconds=max_seconds)):
            if stride > 1 and (src_idx % stride):
                continue
            yield ft

    def _batches(size: int):
        """Group frames into inference batches, preserving order.

        Batching only ever groups the INFERENCE call. Everything downstream
        -- the fatigue analyser, the micro-event detector, both writers --
        still runs strictly one frame at a time in source order, because
        their state is sequential and reordering would corrupt event timing
    ...
    def _process_one(frame, t, detections, out_idx, infer_s, infer_fps):
        """Everything after inference, for one frame, in source order."""
        ...
        preview_img = annotated_frame if want_preview else None
        if preview_img is not None and preview_max_width > 0 and info.width > preview_max_width:
            # The preview is a small pane in the browser, but it was being
            # sent at the source resolution: Streamlit PNG-encodes the array
            # and pushes it over the websocket ~22 times a second, so a
            # 1080p source moved an order of magnitude more data than the
            # pane could ever display. Downscaling here rather than at the
            # call site keeps the full-resolution frame going to the encoder
            # -- only the preview copy shrinks.
            import cv2
            scale = preview_max_width / float(info.width)
            preview_img = cv2.resize(
                annotated_frame,
                (preview_max_width, max(int(round(info.height * scale)), 1)),
    ...
    try:
        out_idx = 0
        for group in _batches(max(int(batch_size), 1)):
            if should_cancel is not None and should_cancel():
                cancelled = True
                break

            t0 = time.perf_counter()
            batch_dets = predict_batch(
                model, [f for f, _ in group], conf=conf, imgsz=imgsz,
                family=family, half=half,
            )
            # Attribute the batch's cost evenly across its frames, so the
            # reported per-frame figure stays comparable with an unbatched
            # run instead of showing one expensive frame then free ones.
            infer_s = (time.perf_counter() - t0) / max(len(group), 1)
            infer_fps = 1.0 / max(infer_s, 1e-6)

            for (frame, t), detections in zip(group, batch_dets):
                yield _process_one(frame, t, detections, out_idx,
                                   infer_s, infer_fps)
                out_idx += 1

...
if run_clicked:
    ...
    use_half = bool(st.session_state.get("use_half", False))
    model = load_model(entry.checkpoint, family=entry.family, resolution=entry.resolution,
                        rfdetr_class=entry.rfdetr_class, half=use_half)

    # Batch size comes from the measured per-model tuning, not a guess.
    # Batching only helps models that are launch-overhead-bound at batch 1;
    # for the larger ones it measurably costs throughput, so an untuned
    # model stays at 1 rather than inheriting another model's optimum.
    from core.autotune import cached_batch
    batch_size = 1 if st.session_state.get("batch_mode") == "off" else (
        cached_batch(entry.key) or 1)

    run_id = uuid.uuid4().hex[:10]
    out_annotated = STATIC_OUT / f"{run_id}_annotated.mp4"
    out_raw = STATIC_OUT / f"{run_id}_raw.mp4"

    # Drop old runs before writing new ones, so the folder cannot grow
    # without bound. Pruning BEFORE the run means the disk space is
    # reclaimed while it is still needed, not after. The run about to
    # start and the one currently on screen are both protected.
    from core.retention import prune_outputs
    _prev = (st.session_state.get(result_key) or {}).get("annotated", "")
    _protect = {run_id}
    if _prev:
        _protect.add(Path(_prev).name.split("_", 1)[0])
    prune_outputs(STATIC_OUT, protect=_protect)
    ...
    toasted_crit = False
    alarm_ph = st.empty()
    # Alert audio is EDGE-triggered on this, not level-triggered. See the
    # comment at the emit site below -- re-emitting per frame pushed the
    # cue's base64 data URI over the websocket on every frame of every
    # alerting segment.
    ...
    for item in gen:
        if isinstance(item, Progress):
            ...
            # Alert audio fires once per TRANSITION into an alert level, not
            # once per frame while that level holds.
            #
            # Emitting per frame re-sent the cue's base64 data URI over the
            # websocket every frame -- 40.3 KB for the critical cue, so about
            # 1.2 MB/s at 30 fps for as long as the alert lasted -- and made
            # the browser instantiate a fresh <audio autoplay> element each
            # time. On a genuinely drowsy clip, where alerts hold for many
            # seconds at a stretch, that is tens of megabytes and thousands of
            # audio elements over one run. It also sounded wrong: thirty
            # overlapping alarm starts per second rather than one alarm.
            #
            # Edge-triggering fixes the resource problem and the behaviour
            # together. Re-entering an alert level after dropping out of it
            # still re-arms, which is the intended alarm semantics.
            level_changed = item.alert_level != last_alert_level
            if level_changed:
                last_alert_level = item.alert_level
            ...
            if item.alert_level == "CRITICAL":
                if not toasted_crit:
                    ...
                    toasted_crit = True
                if level_changed and st.session_state.get("sound_enabled", True):
...
if stored:
    ...
    # The report download sits directly under the scorecard it renders,
    # not only in the download row far below the event table: the report
    # IS the scorecard plus its evidence, so this is where a user who has
    # just read the grade looks for a copy of it.
    _report_html = generate_html_report(
        driver_score, summary, events, source_path.name, entry.display_name
    )
    _rc1, _rc2, _rc3 = st.columns([1.2, 1, 1])
    _rc1.download_button(
        f"📄 {t('video.download_report')}",
        _report_html.encode("utf-8"),
        file_name=(
            f"driver_safety_report_"
            f"{Path(stored['annotated']).name.split('_', 1)[0]}.html"
        ),
        mime="text/html",
        width="stretch",
        type="primary",
        key=f"report_top_{result_key}",
        help=t("video.download_report.help"),
    )

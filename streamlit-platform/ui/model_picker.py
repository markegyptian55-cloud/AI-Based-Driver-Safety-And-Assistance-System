...
def _esc(s: object) -> str:
    """Model display names go into raw HTML -- escape them."""
...
def render_model_picker(entries: dict, *, key: str = "model_key", theme: Theme | None = None) -> str | None:
    ...
    def _acc(k: str) -> float:
        return entries[k].map50_corrected or entries[k].map50 or -1.0

    # Accuracy order is the default, and the top four are called out
    # explicitly above the list. Both matter for the same reason: the
    # number shown on a bounding box is the model's CONFIDENCE, not its
    # accuracy, and the two rank almost inversely here -- RF-DETR-nano
    # shows the highest box confidence in the registry (76.1 % mean) on
    # 78.98 % mAP, while the most accurate model (YOLO11m warm-start,
    # 86.75 %) shows only 63.9 %. Left to pick by what looks confident
    # on screen, a user reliably chooses a worse model.
    ranked = sorted(entries.keys(), key=_acc, reverse=True)
    top4 = ranked[:4]

    st.markdown(f'<span class="nb-eyebrow">{t("side.model")}</span>', unsafe_allow_html=True)

    rows = "".join(
        f'<div class="nb-top4-row">'
        f'<span class="nb-top4-rank">{i}</span>'
        f'<span class="nb-top4-name">{_esc(entries[k].display_name)}</span>'
        f'<span class="nb-top4-acc nb-ltr">{_acc(k) * 100:.2f}%</span>'
        f'</div>'
        for i, k in enumerate(top4, 1)
    )
    st.markdown(
        f'<div class="nb-top4">'
        f'<div class="nb-top4-title">{t("side.top4")}</div>'
        f'{rows}'
        f'<div class="nb-top4-note">{t("side.top4.note")}</div>'
        f'</div>',
        unsafe_allow_html=True,
    )

    sort_by_accuracy = st.toggle(t("side.sort_accuracy"), value=True, key=f"{key}_sort_accuracy")
    keys = ranked if sort_by_accuracy else list(entries.keys())

    def _label(k: str) -> str:
        # Medal marks the same four as the box above, so the callout and
        # the list cannot visually disagree about which models they are.
        rank = top4.index(k) + 1 if k in top4 else None
        return f"🏅 {rank}. {entries[k].display_name}" if rank else entries[k].display_name
